...
const CurrentPackagePage = () => {
  ...
  useEffect(() => {
    const fetchCurrentPackage = async () => {
      try {
        ...
        setError(err.response?.data?.error || 'Không thể tải thông tin gói hiện tại');
      } finally {
        setLoading(false);
      }
    };
  ...
  const calculateProgress = () => {
    if (!data) return 0;
    const totalDays = dayjs(data.expiresAt).diff(dayjs(data.purchasedAt), 'day');
  ...
  if (!data?.hasActivePackage) {
    return (
      <Container maxWidth="md" sx={{ py: 4, textAlign: 'center' }}>
        ...
        <Button variant="contained" size="large" onClick={() => navigate('/packages')}>
          Chọn gói ngay
        </Button>
      </Container>
    );
              ...
              <Box sx={{ bgcolor: '#f5f5f5', p: 3, borderRadius: 2 }}>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                  ...
                  <Typography fontWeight="bold">
                    {data.package.postLimit - data.postsLeft}/{data.package.postLimit}
                  </Typography>
                </Box>

                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 3 }}>
                  <Typography>Số bài còn lại:</Typography>
                  <Typography fontWeight="bold" color={data.postsLeft === 0 ? 'error' : 'primary'}>
                    {data.postsLeft}
                  </Typography>
                </Box>
